...
class absenteeism_model():
        ...
        def load_and_clean_data(self, data_file):
            ...
            column_names = ['Date', 'Transportation Expense', 'Distance to Work', 'Age',
                           ...
                           'Pets', 'Absenteeism Time in Hours', 'Reason_Type1', 'Reason_Type2', 'Reason_Type3', 'Reason_Type4']
            df.columns = column_names

            # re-order the columns in df
            column_names_reordered = ['Reason_Type1', 'Reason_Type2', 'Reason_Type3', 'Reason_Type4', 'Date', 'Transportation Expense', 
            ...
            # re-order the columns in df
            column_names_upd = ['Reason_Type1', 'Reason_Type2', 'Reason_Type3', 'Reason_Type4', 'Month Value', 'Day of the Week',
                                'Transportation Expense', 'Distance to Work', 'Age',
                                'Daily Work Load Average', 'Body Mass Index', 'Education', 'Children',
                                'Pets', 'Absenteeism Time in Hours']
            df = df[column_names_upd]

            df['Reason_Type1'] = df['Reason_Type1'].map({True:1,False:0})
            df['Reason_Type2'] = df['Reason_Type2'].map({True:1,False:0})
            df['Reason_Type3'] = df['Reason_Type3'].map({True:1,False:0})
            df['Reason_Type4'] = df['Reason_Type4'].map({True:1,False:0})
